 ...
 const getById = async (req, res, next) => {
   try {
    const userId = req.user.id
      ...
      // console.log(contact)
      if (contact) {
        return res.status(HttpCode.OK).json({status: 'succes', code: HttpCode.OK, data: {contact}})
      }
  ...
  const addContact = async (req, res, next) => {
    try {
      ...
      return res.status(HttpCode.CREATED).json({status: 'succes', code: HttpCode.CREATED, data: {contact}})
    } catch (error) {
  ...
  const removeContact = async (req, res, next) => {
    try {
      ...
      if (contact) {
        return res.status(HttpCode.OK).json({status: 'succes', code: HttpCode.OK,  message: 'contact deleted'})
      }
      return  res.status(HttpCode.NOT_FOUND).json({status: 'error', code: HttpCode.NOT_FOUND, message: 'Not Found'})
    ...
    const updateStatusContact =  async (req, res, next) => {
      ...
      const contact = await Contacts.updateContact(userId, req.params.contactId, req.body)
        if (contact) {
         ...
         return  res.status(HttpCode.NOT_FOUND).json({status: 'error', code: HttpCode.NOT_FOUND, message: 'Missing field favorite'})
    } catch (err) {
      next(err)
    }
}
